...
After this, we can see in our shell the webconsole-config.yaml, we only need to navegate untill *extensions* and add the url for our own `javascript` in the *scriptURLs* section.
